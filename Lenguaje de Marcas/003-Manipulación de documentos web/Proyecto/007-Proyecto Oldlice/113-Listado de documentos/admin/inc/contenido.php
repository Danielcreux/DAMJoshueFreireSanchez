...
while ($fila = $resultado->fetch_assoc()) {		// Para cada resultado obtenido
	...
	foreach($fila as $clave=>$valor){				// Para cada uno de los campos
		...
		if($valor !== null && is_string($valor) && strlen($valor) < 300)  {						// Si el campo es menor que 300 caracteres
  			echo "<td tabla='".$_GET['tabla']."'
                    columna = '".$clave."'
                    Identificador ='".$identificador."'>".htmlspecialchars($valor)."</td>";				// Pongo el valor en una columna
  		}else if($valor !== null) {												// En caso contrario
  			echo "<td>
